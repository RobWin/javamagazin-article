...
Der Fokus der Dokumentation und der generierten Inhaltsangabe liegt auf der Funktionalität, die von der API angeboten wird, und nicht auf den konkreten API Endpunkten.

Swagger2Markup hat viele Konfigurationsparameter und kann über verschiedene Wege konfiguriert werden, z.B. System Properties, Properties-Datei, Fluent-API oder einer Java Map. Listing 4 zeigt beispielsweise die Verwendung der Fluent-API, um das Ausgabeformat und die Ausgabesprache zu ändern. Derzeit werden als Sprachen Englisch, Deutsch, Russisch und Türkisch unterstützt.
